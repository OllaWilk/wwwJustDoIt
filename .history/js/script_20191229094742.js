...
document.querySelector('form').addEventListener('submit', function(event) {
    ...
    /*skype walidation*/
    let skypeInput = event.target.querySelector('input[name="name"]')
    if(skypeInput.value.lenght==0) {
        isFormValidate = false;
        skypeInput.parentElement.querySelector('.error').innerHTML = 'Name must be filled out';
    } 

    /*icq walidation*/
    let icqInput = event.target.querySelector('input[name="email"]')
    if(icqInput.value.indexOf('@') < 0) {
        isFormValidate = false;
        icqInput.parentElement.querySelector('.error').innerHTML = 'Incorect e-mail adress';
    }

    /*msn walidation*/
    let msnInput = event.target.querySelector('input[name="email"]')
    if(msnInput.value.indexOf('@') < 0) {
        isFormValidate = false;
        msnInput.parentElement.querySelector('.error').innerHTML = 'Incorect e-mail adress';
    }

    /*password walidation*/
    let passwordInput = event.target.querySelector('input[name="email"]')
    if(passwordInput.value.indexOf('@') < 0) {
        isFormValidate = false;
        passwordInput.parentElement.querySelector('.error').innerHTML = 'Incorect e-mail adress';
    }

    /*repeat walidation*/
    let repeatInput = event.target.querySelector('input[name="email"]')
    if(repeatInput.value.indexOf('@') < 0) {
        isFormValidate = false;
        repeatInput.parentElement.querySelector('.error').innerHTML = 'Incorect e-mail adress';
    }

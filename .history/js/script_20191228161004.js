...
document.querySelector('form').addEventListener('submit', function(event) {
    ...
    if(nameInput.value.indexOf('1') < 0) {
        isFormValidate = false;
       nameInput.parentElement.querySelector('.error').innerHTML = 'Incorect e-mail adress';
    }
...
document.querySelector('form').addEventListener('submit', function(event){
    ...
    let name = document.querySelector('input[name="email"]').value;
    if(!regName.test(name)){
        isFormValidate = false;
        emailAddressInput.parentElement.querySelector('.error').innerHTML = 'Incorect name ';
    }
})

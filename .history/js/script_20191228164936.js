...
document.querySelector('form').addEventListener('submit', function(event) {
    ...
    if (nameInput.lenght === 0) {
        isFormValidate = false;
        nameInput.parentElement.querySelector('.errorText').innerHTML = 'Name must be filled out';
    }
...
document.querySelector('form').addEventListener('submit', function(event) {
    ...
    if(phoneInput.value.match(phoneNumber)) 
    {
        ...
        {
            isFormValidate = false;
            phoneInput.parentElement.querySelector('.errorPhone').innerHTML = 'Incorect phone number';
        }
